...
class MonteCarloSearchTreeContinuous:
    def __init__(self, env):
        self.env = env
        self.root = NodeContinuous((0,0))
    ...
    def _expansion(self, node):
        for action in self.env.actions:
            ...
            child = NodeContinuous(next_state, parent=node)
    ...
    def _simulation(self, node):
        ...
        while not self.check_goal(current_state) and depth < self.depth_limit:
    ...
    def check_goal(self,state):
        if -np.cos(state[0]) - np.cos(state[0]+state[1]) > 1.0:
            return True
        return False